...
{
	if (1 == lock)
		mapphone_lock_dev_cpufreq(NULL, MAPPHONE_CPU_CLK_LOCK);
	else
		mapphone_lock_dev_cpufreq(NULL, MAPPHONE_CPU_CLK_UNLOCK);
}
...
static void mapphone_lock_dev_cpufreq(struct device *dev, int lock)
{
	...
	if (MAPPHONE_CPU_CLK_LOCK == lock) {
		resource_request("vdd1_max", &dummy_vdd1_dev,
			(omap_pm_get_max_vdd1_opp() - 1));
		...
		if (flag == 1) {
			...
			resource_release("vdd1_max", &dummy_vdd1_dev);
			flag = 0;
		}
	}
}
