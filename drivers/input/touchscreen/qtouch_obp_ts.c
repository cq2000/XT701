...
static struct qtouch_ts_data *gts;
...
void set_accy_status(unsigned char accy_status)
{
	...
	pr_info("%s: Accy Status = %d\n", __func__, accy_status);
	ret = qtouch_power_config(gts, 0);
	if (ret < 0)
		pr_err("%s: Cannot write power config\n", __func__);
	obj = find_obj(gts, QTM_OBJ_TOUCH_MULTI);

	 /* changes tch_det_thr's value*/
	addr =
	    obj->entry.addr + offsetof(struct qtm_touch_multi_cfg,
	    tch_det_thr);
	if (accy_status)
		val = 0x24; /* 0x12 */
	else
		val = gts->pdata->multi_touch_cfg.tch_det_thr;
	pr_info("%s: tch_det_thr address = 0x%x, val = %d\n",
		__func__, addr, val);
	ret = qtouch_write_addr(gts, addr, &val, 1);
	if (ret)
		pr_err("%s: Unable to send the reset msg\n", __func__);

	 /* changes mov_hyst_next's value*/
	addr =
	    obj->entry.addr + offsetof(struct qtm_touch_multi_cfg,
	    mov_hyst_next);
	if (accy_status)
		...
		val = gts->pdata->multi_touch_cfg.mov_hyst_next;
	pr_info("%s: mov_hyst_next address = 0x%x, val = %d\n",
		__func__, addr, val);
	ret = qtouch_write_addr(gts, addr, &val, 1);
	if (ret)
		pr_err("%s: Unable to send the reset msg\n", __func__);
	ret = qtouch_power_config(gts, 1);
	if (ret < 0)
		pr_err("%s: Cannot write power config\n", __func__);

	/* Forcing calibration */
	ret = qtouch_force_calibration(gts);
	if (ret != 0)
		pr_err("%s: Unable to recalibrate after reset\n", __func__);
	/* reset the address pointer */
	ret = qtouch_set_addr(gts,
		gts->obj_tbl[QTM_OBJ_GEN_MSG_PROC].entry.addr);
	if (ret != 0)
		pr_err("%s: Unable to reset address pointer after reset\n",
		       __func__);
}
EXPORT_SYMBOL(set_accy_status);
...
/*begin set number of multitouch points */
...
/*end set number of multitouch points */
